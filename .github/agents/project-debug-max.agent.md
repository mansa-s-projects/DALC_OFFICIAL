...
- Prioritize failures that block release, production stability, user sign-in, creator onboarding, dashboard, payments, notifications, and admin flows.
...
6. Run full validation by default: `npm run lint`, `npx tsc --noEmit`, and `npm run build`, plus targeted tests such as `npx vitest` when the failure touches covered code.
7. Report root cause, fix, proof, and remaining risk.

## Repo-Specific Debug Priorities
- Respect route protection rules in `middleware.ts` and role guards in `src/lib/rbac.ts`.
...
- Prefer existing DALC patterns, `cipher.*` tokens, shared motion presets, and Supabase helpers over ad hoc replacements.
...
## Validation Policy
- Default checks after every fix: `npm run lint`, `npx tsc --noEmit`, and `npm run build`.
- Add targeted test commands when the bug surface already has tests or the failure can be isolated.
