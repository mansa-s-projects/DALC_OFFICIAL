...
export default function AdminBusiness() {
  ...
  const bizRequests = allRequests.filter((r) => r.category === 'business');

  const filtered = bizRequests.filter((r) => {
  ...
  const inProgressCount = bizRequests.filter((r) => r.status === 'in_progress' || r.status === 'assigned').length;
  const quotedCount = bizRequests.filter((r) => r.status === 'quoted').length;
  const confirmedCount = bizRequests.filter((r) => r.status === 'confirmed').length;

  const handleStatusChange = (requestId: string, newStatus: RequestStatus) => {
    const current = bizRequests.find((r) => r.id === requestId);
    if (!current) return;
  ...
  return (
    <div>
      <AdminPageHeader title="Business Queue" />

      <div className="grid grid-cols-2 gap-4 mb-8 lg:grid-cols-4">
        <AdminStatCard label="Total" value={bizRequests.length} icon={Briefcase} colorClass="text-[#C8A46B]" delay={0} />
        <AdminStatCard label="In Progress" value={inProgressCount} icon={TrendingUp} colorClass="text-amber-400" delay={0.05} />
        <AdminStatCard label="Quote Ready" value={quotedCount} icon={Building2} colorClass="text-blue-400" delay={0.1} />
        <AdminStatCard label="Confirmed" value={confirmedCount} icon={CheckCircle} colorClass="text-emerald-400" delay={0.15} />
      </div>

      <div className="flex flex-wrap gap-3 mb-8">
        <AdminSearchInput value={search} onChange={setSearch} placeholder="Search business requests..." />
        <AdminSelectFilter
          value={filterStatus}
      ...
      {isLoading ? (
        ...
        <AdminEmptyState message="No business requests found." />
      ) : (
                  ...
                  <div className="flex items-center gap-4 flex-1 min-w-0">
                    <div className="min-w-0 flex-1">
                      <p className="text-white font-bold truncate">
                        {request.title || request.venue_name || 'Business Service'}
                      </p>
                ...
                {isExpanded && (
                  <div className="border-t border-white/5 p-6 bg-white/[0.01]">
                    <div className="grid grid-cols-2 gap-4 mb-6 text-sm lg:grid-cols-3">
                      ...
                      <div>
                        <span className="text-gray-500 text-xs uppercase tracking-wider block mb-1">Service</span>
                        <span className="text-white">{request.venue_name || request.title || '—'}</span>
                      </div>
                      <div>
