...
import { useVenue } from '../../../features/nightlife/hooks/useVenue';
...
export default function VenueDetailPage({ params }: { params: { id: string } }) {
  ...
  const { data: venue, isLoading } = useVenue(params.id);
  ...
  useEffect(() => {
    if (!isBookingModalOpen) {
      const timer = setTimeout(() => {
        setBookingStep(1);
        setBookingForm({ date: '', guests: '2', name: '', contact: '', notes: '' });
      }, 300);
      return () => clearTimeout(timer);
    }
  }, [isBookingModalOpen]);

  const allImages = venue ? [venue.hero_image, ...venue.gallery_images] : [];
  ...
  const prevHeroImage = useCallback(() => {
    setCurrentHeroIndex((prev) => (prev - 1 + allImages.length) % Math.max(allImages.length, 1));
  }, [allImages.length]);
  ...
  const handleBookingSubmit = async () => {
    ...
    try {
      await createRequest.mutateAsync({
        ...
        party_size: bookingForm.guests === 'large' ? 20 : Number(bookingForm.guests) || 2,
  ...
  if (!venue) return (
    <div className="min-h-screen bg-luxury-black flex items-center justify-center">
      <div className="text-white text-center">
        ...
        <Link href="/nightlife" className="text-luxury-gold mt-4 block hover:underline">Return to Nightlife</Link>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-luxury-black">
      ...
      <div className="relative h-[80vh] w-full overflow-hidden group">
        <AnimatePresence mode="popLayout">
          ...
          >
            <img src={allImages[currentHeroIndex]} alt={venue.name} className="w-full h-full object-cover" />
        ...
        <div className="absolute bottom-0 left-0 w-full p-8 md:p-12 max-w-7xl mx-auto z-10">
          <Link href={`/nightlife`} className="flex items-center gap-2 text-white/60 hover:text-luxury-gold mb-8 transition-colors w-fit">
            <ArrowLeft className="w-4 h-4" /> Back to Nightlife
          </Link>
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
            ...
            >
              ...
              <h1 className="text-6xl md:text-8xl font-display text-white mb-6 leading-tight tracking-tight">{venue.name}</h1>
              ...
              <div className="flex flex-wrap items-center gap-6 text-gray-300 text-sm md:text-base font-light">
                ...
                <div className="flex gap-1 text-luxury-gold">
                  ...
                  <span className="text-gray-600">{'$$$$'.slice(venue.price_tier)}</span>
                </div>
              </div>
            </motion.div>
            ...
            >
              <button
               ...
               >
                   View Insider Guide <ArrowDown className="w-4 h-4 group-hover:translate-y-1 transition-transform" />
              </button>
      ...
      <div className="max-w-7xl mx-auto px-4 py-20 grid grid-cols-1 lg:grid-cols-12 gap-16">
        ...
        <div className="lg:col-span-7 space-y-20">
          ...
          >
            <h3 className="text-xl font-display text-white mb-8">Booking Intelligence</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-y-10 gap-x-12">
               <div className="flex gap-4">
                  ...
                  <div>
                     <h4 className="text-xs font-bold uppercase text-gray-500 mb-1 tracking-wider">Best Time to Go</h4>
                     <p className="text-white">{venue.best_time || 'Dinner Service (8:00 PM)'}</p>
                  </div>
               </div>
               ...
               <div className="flex gap-4">
                  ...
                  <div>
                     <h4 className="text-xs font-bold uppercase text-gray-500 mb-1 tracking-wider">Perfect For</h4>
                     <p className="text-white">{venue.who_its_for || 'Special Occasions & Couples'}</p>
                  </div>
               </div>
               ...
               <div className="flex gap-4">
                  ...
                  <div>
                     <h4 className="text-xs font-bold uppercase text-gray-500 mb-1 tracking-wider">Dress Code</h4>
               ...
               {venue.insider_tip && (
                  <div className="col-span-1 md:col-span-2 mt-4 pt-6 border-t border-white/5">
                     <div className="flex gap-4">
                        ...
                        <div>
                           <h4 className="text-xs font-bold uppercase text-luxury-gold mb-1 tracking-wider">Concierge Tip</h4>
                           <p className="text-white italic font-light">"{venue.insider_tip}"</p>
                        </div>
                     </div>
                  </div>
               )}
          ...
          <motion.section
            ...
            viewport={{ once: true }}
          >
            <h2 className="text-xl font-display text-white mb-6">Signature Features</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {venue.highlights.map((highlight, i) => (
                <div key={i} className="flex items-center gap-4 p-5 border border-white/5 rounded-sm bg-luxury-charcoal hover:bg-white/[0.05] transition-colors group">
                  <div className="w-1 h-8 bg-luxury-gold/20 group-hover:bg-luxury-gold transition-colors" />
                  <span className="text-gray-200 font-medium tracking-wide">{highlight}</span>
                </div>
              ))}
            </div>
          </motion.section>
          ...
          <motion.section
             ...
             viewport={{ once: true }}
          >
            <h2 className="text-xl font-display text-white mb-6">Visual Journey</h2>
        ...
        >
          ...
          <div className="sticky top-24 space-y-8">
             ...
             <div className="glass-panel p-8 rounded-sm border border-white/10 bg-luxury-black/50 backdrop-blur-xl">
               <h3 className="text-xl font-display text-white mb-6">Reservation Details</h3>

               <div className="space-y-6">
                 <div className="flex justify-between items-center text-sm border-b border-white/5 pb-4">
                   <span className="text-gray-400">Opening Hours</span>
                   <span className="text-white text-right max-w-[50%]">{venue.opening_hours}</span>
                 </div>
                 <div className="flex justify-between items-center text-sm border-b border-white/5 pb-4">
                   <span className="text-gray-400">Cancellation</span>
                   <span className="text-white text-right max-w-[50%]">24h Notice Required</span>
                 </div>
                 <div className="flex justify-between items-center text-sm pb-2">
                   <span className="text-gray-400">Deposit</span>
                   <span className="text-white text-right">Required for 6+ guests</span>
                 </div>
               </div>
          ...
          >
            ...
            >
              ...
              <div className="bg-white/5 p-6 flex justify-between items-center border-b border-white/5">
                <div>
                  <h3 className="text-luxury-gold text-xs font-bold uppercase tracking-widest mb-1">Reservation Request</h3>
                  <h2 className="text-xl font-display text-white">{venue.name}</h2>
                </div>
                <button
              ...
              <div className="p-8">
                  ...
                  <form onSubmit={(e) => { e.preventDefault(); setBookingStep(2); }} className="space-y-6">
                    ...
                    <div className="p-4 bg-luxury-gold/10 border border-luxury-gold/20 rounded-sm flex gap-3 items-start">
                       ...
                       <div className="text-xs text-gray-400 leading-relaxed">
                          <span className="text-luxury-gold font-bold block mb-1">Concierge Guarantee</span>
                          Your request will be prioritized. Availability is subject to venue confirmation within 30 minutes.
                       </div>
                    </div>
                ...
                {bookingStep === 2 && (
                  <form onSubmit={(e) => { e.preventDefault(); handleBookingSubmit(); }} className="space-y-6">
                     ...
                     <div>
                        <label className="block text-xs font-bold uppercase text-gray-500 mb-2">Contact (WhatsApp / Email)</label>
                        <input
                          type="text"
                          required
                          placeholder="+971..."
                     ...
                     <div className="flex gap-4">
                       ...
                       >
                         {createRequest.isPending ? 'Submitting...' : 'Confirm Request'}
                       </button>
                     </div>
                  </form>
                )}
                ...
                {bookingStep === 3 && (
                  <div className="text-center py-8">
                     ...
                     <h3 className="text-2xl font-display text-white mb-2">Request Sent</h3>
                     <p className="text-gray-400 mb-8 max-w-xs mx-auto text-sm leading-relaxed">
                        Our concierge team has received your request for <strong>{venue.name}</strong>. We will contact you shortly to confirm your table.
                     </p>
              ...
              {bookingStep < 3 && (
                <div className="absolute bottom-0 left-0 h-1 bg-luxury-gold transition-all duration-300" style={{ width: bookingStep === 1 ? '50%' : '100%' }} />
              )}
            </motion.div>
          </motion.div>
        )}
