...
const FAQS = [
  ...
  },
  {
    ...
    a: 'Private jets primarily use Al Maktoum International (DWC) and the Executive Flight Terminals at Dubai International (DXB).',
  },
  {
    q: 'Are meals included on private jet charters?',
    a: 'Standard charters include light refreshments. Gourmet catering can be arranged with advance notice.',
  },
  {
    q: 'How much luggage can I bring?',
    a: 'Luggage capacity varies by aircraft. Light jets accommodate 5–6 bags, while ultra-long-range jets handle 15+ large suitcases.',
  },
];
...
export default function TravelJets() {
  const [showFilters, setShowFilters] = useState(false);
  const [openFaq, setOpenFaq] = useState<number | null>(null);
  const [filters, setFilters] = useState<FilterType>({ subcategory: 'jets' });

  const { data: services = [], isLoading } = useTransportServices(filters);

  const hasFilters = Boolean(
    filters.sub_subcategory || filters.pricing_model ||
    filters.price_min || filters.price_max || filters.availability_type
  );
    ...
    <div className="min-h-screen bg-luxury-black">
      ...
      <section className="relative h-[50vh] min-h-[400px] flex flex-col items-center justify-center pt-20 px-4 text-center overflow-hidden">
        ...
        >
          ...
          <h1 className="text-4xl md:text-6xl font-display text-white mb-4">Private Jets</h1>
          <p className="text-gray-300 text-base max-w-xl mx-auto leading-relaxed">
            Charter world-class aircraft for business or leisure. Seamless travel from Dubai to anywhere in the world.
          </p>
        </motion.div>
      </section>
          ...
          <div className="lg:col-span-3">
            <div className="flex items-center justify-between mb-6">
              <p className="text-gray-500 text-sm">
                {isLoading ? 'Loading...' : (
                  <><span className="text-white font-medium">{services.length}</span> aircraft available</>
                )}
              </p>
              {hasFilters && (
                <button
                  onClick={() => setFilters({ subcategory: 'jets' })}
