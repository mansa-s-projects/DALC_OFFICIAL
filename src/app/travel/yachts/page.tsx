...
import { ArrowRight, X, Anchor, Waves, Sun, Wine, MapPin } from 'lucide-react';
...
export default function TravelYachts() {
  const [showFilters, setShowFilters] = useState(false);
  const [openFaq, setOpenFaq] = useState<number | null>(null);
  const [filters, setFilters] = useState<FilterType>({ subcategory: 'yachts' });

  const { data: services = [], isLoading } = useTransportServices(filters);

  const hasFilters = Boolean(
    filters.sub_subcategory || filters.pricing_model ||
    filters.price_min || filters.price_max || filters.availability_type || filters.capacity_min
  );

  const clearFilters = () => setFilters({ subcategory: 'yachts' });
    ...
    <div className="min-h-screen bg-luxury-black">
      ...
      <section className="relative h-[50vh] min-h-[400px] flex flex-col items-center justify-center pt-20 px-4 text-center overflow-hidden">
        ...
        >
          ...
          <h1 className="text-4xl md:text-6xl font-display text-white mb-4">Yacht Charter</h1>
          <p className="text-gray-300 text-base max-w-xl mx-auto leading-relaxed">
            Cruise the Arabian Gulf in style. From intimate day cruises to overnight luxury charters with full crew.
          </p>
        </motion.div>
      </section>
      ...
      <section className="border-y border-white/10 bg-white/[0.02]">
        <div className="max-w-7xl mx-auto px-4 md:px-8 py-8">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
            {[
              { icon: <Waves className="w-5 h-5 text-luxury-gold" />, label: 'Calm Waters' },
              { icon: <Sun className="w-5 h-5 text-luxury-gold" />, label: 'Year-Round' },
              { icon: <Anchor className="w-5 h-5 text-luxury-gold" />, label: 'Multiple Marinas' },
              { icon: <Wine className="w-5 h-5 text-luxury-gold" />, label: 'Full Service' },
            ].map(({ icon, label }) => (
          ...
          <div className="lg:col-span-3">
            <div className="flex items-center justify-between mb-6">
              <p className="text-gray-500 text-sm">
                {isLoading ? 'Loading...' : (
                  <><span className="text-white font-medium">{services.length}</span> yacht{services.length !== 1 ? 's' : ''} available</>
                )}
              </p>
              {hasFilters && (
                <button onClick={clearFilters} className="flex items-center gap-1.5 text-xs text-gray-400 hover:text-luxury-gold transition-colors">
            ...
            {isLoading ? (
              ...
              <div className="text-center py-24 border border-white/10">
                ...
                {hasFilters && (
                  <button onClick={clearFilters} className="px-6 py-2 border border-luxury-gold/30 text-luxury-gold text-xs uppercase tracking-widest hover:bg-luxury-gold/10 transition-colors">
                    Clear filters
                  </button>
                )}
              </div>
      ...
      <section className="max-w-7xl mx-auto px-4 md:px-8 pb-16">
        <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="mb-8">
          <p className="text-luxury-gold text-xs font-bold uppercase tracking-[0.4em] mb-3">Popular Routes</p>
            ...
            >
              ...
              <div className="absolute bottom-0 left-0 right-0 p-4">
                ...
                <p className="text-gray-400 text-xs flex items-center gap-1">
                  <MapPin className="w-3 h-3" /> {route.duration}
                </p>
              </div>
            </motion.div>
          ))}
          ...
          {FAQS.map((faq, idx) => (
            <div key={idx} className="border border-white/10">
              ...
              >
                ...
                <span className={`text-luxury-gold transition-transform duration-300 ${openFaq === idx ? 'rotate-180' : ''}`}>▼</span>
              </button>
              {openFaq === idx && (
                <motion.div
      ...
      <section className="max-w-7xl mx-auto px-4 md:px-8 pb-24">
        <div className="border border-luxury-gold/20 p-8 md:p-12 flex flex-col md:flex-row items-center justify-between gap-6">
          <div>
            <p className="text-luxury-gold text-xs font-bold uppercase tracking-widest mb-2">Bespoke Charters</p>
            <h3 className="text-white font-display text-xl md:text-2xl">Planning a special celebration?</h3>
          </div>
          <Link
            href="/request"
