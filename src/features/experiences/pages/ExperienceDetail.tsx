...
import { useCheckCapacity, useCreateExperienceBooking } from '../hooks/useExperienceBooking';
    ...
    return (
      <div className="min-h-screen bg-luxury-black flex flex-col">
        ...
        <div className="flex-1 flex items-center justify-center flex-col gap-4">
          ...
          <Link to={`/experiences/${subcategory}`} className="text-luxury-gold text-sm underline">
            Back to {subcategory}
          </Link>
        </div>
        <Footer />
      ...
      <section className="relative h-[70vh] min-h-[500px] overflow-hidden">
        ...
        <div className="absolute top-24 left-0 right-0 px-4 md:px-8 max-w-7xl mx-auto">
          <nav className="flex items-center gap-2 text-xs text-gray-400 uppercase tracking-widest">
            ...
            <span>/</span>
            <Link to={`/experiences/${subcategory}`} className="hover:text-luxury-gold transition-colors">
                ...
                >
                  ...
                  {!session?.user && (
                    <p className="text-gray-500 text-xs text-center">
                      Please <Link to="/login" className="text-luxury-gold underline">sign in</Link> to book
                    </p>
                  )}
                </motion.div>
              )}
        ...
        <div className="border border-luxury-gold/20 p-8 md:p-12 flex flex-col md:flex-row items-center justify-between gap-6">
          <div>
            ...
            <h3 className="text-white font-display text-xl md:text-2xl">
              Explore all {SUBCATEGORY_LABELS[experience.subcategory]} experiences
            </h3>
          </div>
          <Link
            to={`/experiences/${experience.subcategory}`}
