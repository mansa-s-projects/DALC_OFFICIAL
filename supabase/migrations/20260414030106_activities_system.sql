...
CREATE POLICY "Anyone can view active pricing"
  ON public.activity_pricing FOR SELECT USING (is_active = true);
...
CREATE POLICY "Anyone can view active availability"
  ON public.activity_availability FOR SELECT USING (is_active = true);
...
CREATE POLICY "Users view own bookings"
  ON public.activity_bookings FOR SELECT USING (user_id = auth.uid());

CREATE POLICY "Users create own bookings"
  ON public.activity_bookings FOR INSERT WITH CHECK (user_id = auth.uid());
